...
shinyUI(navbarPage(title = "Convenio PAS-DINARA", inverse = TRUE, collapsable = TRUE, fluid = TRUE, responsive = TRUE,
                            ...
                            div(class="outer",
                                ...
                                absolutePanel(id = "controls",
                                              ...
                                              checkboxGroupInput(inputId = 'capas',
                                                                 label = "",
                                                                 choices = c('Isóbatas' = 'isobatas',
                                                                             'Estaciones de mejillón' = 'est_mej',
                                                                             'Pecios' = 'pecios',
                                                                             'Densidad AIS (2012-2013)' = 'dens_ais',
                                                                             'Navegación sonar' = 'nav_son')
                                                                 )
                                              )
                                ),
                   ...
                   tabPanel("Cargar SBL", icon=icon(name="file", "fa-2x"),
                            sidebarPanel(
                              ...
                              conditionalPanel(condition="input.extraer == 'gps'", strong('Resolución'), selectInput(inputId='res', '', choices=c('Max' = 0, '1 min' = 1, '2 min' = 2, '3 min' = 3, '4 min' = 4, '5 min' = 5), multiple=FALSE),
                                               tags$hr(), 
                                               ...
                                               conditionalPanel(condition = "input.exportar == 'tabla' && input.exportar == 'shapefile'",
                                                          ...
                                                          tags$hr(),
                                                          helpText("Nota: se sugiere utilizar nombres que contengan","un identificador de la campaña y la fecha de la misma."))
                                               )
                              ),
                            mainPanel(
                   ...
                   tabPanel("Acerca de esta APP", icon = icon(name = "comments", "fa-2x"),
                            ...
                            mainPanel(
                              tabPanel("Acerca de esta APP",
                                       ...
                                       p(style="text-align:justify",'Está siendo desarrollada en el marco del convenio entre la Dirección Nacional de Recursos Acuáticos (DINARA) y el
                                         Programa de Arqueología Subacuática (PAS). Esta aplicación web le permite al usuario seguir las actividades realizadas entre el PAS y la DINARA.
                                         Además brinda la posibilidad de visualizar y descargar la información almacenada en los archivos exportados en bruto del sonar de barrido lateral
                                         StarFish 990F de Tritech. También brinda la posibilidad de visualizar la información en un mapa interactivo. El objetivo de esta aplicación es
                                         generar una herramienta que permita reunir toda la información generada a través del convenio y facilitar su acceso mediante una interfaz amigable
                                         e interactiva; además de proporcionar herramientas que resulten de utilidad a los técnicos a la hora de procesar datos de sonar luego de cada
                                         campaña.'),
                                       tags$hr(),
                                       h3(p(strong('Características'))),
                                       p(style="text-align:justify",'La mayor parte del software empleado para desarrollar esta aplicación es libre, eso quiere decir que garantiza al
                                         usuario la libertad de poder usarlo, estudiarlo, compartirlo (copiarlo) y modificarlo. El software R es un proyecto de software libre que es
                                         colaborativo y tiene muchos contribuyentes.'),
                                       tags$hr(),
                                       h3(p(strong('Guía de usuario'))),
                                       HTML('<div style="clear: left;"><img src="https://raw.githubusercontent.com/guzmanlopez/Pampero/master/Figuras/PDF.png" alt="" style="width: 5%;
                                            height: 5%; float: left; margin-right:5px" /></div>'),
                                       br(),
